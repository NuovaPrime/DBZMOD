            ...
            if (!player.channel || (ChargeLevel >= ChargeLimit))
            {
                if (ChargeLevel >= 1)
                {
                    ...
                    for (int i = 0; i < 100; i++)
                    {
                        ...
                        Dust tDust = Dust.NewDustDirect(projectile.position + (position * (20 + 3.0f * projectile.scale)), projectile.width, projectile.height, 107, 0f, 0f, 213, default(Color), 3.0f);
