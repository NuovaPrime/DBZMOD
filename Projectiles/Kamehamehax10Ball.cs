        ...
        public override void AI()
        {
            ...
            if (!player.channel || (ChargeLevel >= ChargeLimit))
            {
                if (ChargeLevel >= 1)
                {
                    ...
                    Projectile.NewProjectileDirect(new Vector2(projectile.Center.X, projectile.Center.Y), new Vector2((float)((Math.Cos(rot) * 15)), (float)((Math.Sin(rot) * 15))), mod.ProjectileType("Kamehamehax10Blast"), projectile.damage + (ChargeLevel * 65), projectile.knockBack, projectile.owner);
                    ...
                    for (int i = 0; i < 100; i++)
                    {
                        ...
                        Dust tDust = Dust.NewDustDirect(projectile.position + (position * (20 + 3.0f * projectile.scale)), projectile.width, projectile.height, 15, 0f, 0f, 213, color, 3.0f);
