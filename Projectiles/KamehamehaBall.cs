        ...
        public override void AI()
        {
            ...
            if (!player.channel || (ChargeLevel >= ChargeLimit))
            {
                if (ChargeLevel >= 1)
                {
                    ...
                    Projectile.NewProjectileDirect(new Vector2(projectile.Center.X, projectile.Center.Y), new Vector2((float)((Math.Cos(rot) * 10)), (float)((Math.Sin(rot) * 10))), mod.ProjectileType("KamehamehaBlast"), projectile.damage + (ChargeLevel * 30), projectile.knockBack, projectile.owner);
